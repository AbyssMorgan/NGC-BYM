	...
	public class CreepBase extends MonsterBase
	{
		...
		override protected function tickState(param1:int = 1) : Boolean
		{
			...
			if(_gridMoveCounter >= GRID_UPDATE_INTERVAL)
			{
				...
				if(!_lastGridPosition || _lastGridPosition.x != currentGridX || _lastGridPosition.y != currentGridY)
				{
		...
		public function findHealingTargets() : void
		{
			...
			_targetCreeps = Targeting.getCreepsInRange(500,_tmpPoint,attackFlags,this);
			if(_targetCreeps.length > 0)
			{
				_targetCreeps = GLOBAL.shuffle_array(_targetCreeps);
		...
		public function findDefenseTargets() : void
		{
			...
			_targetCreeps = Targeting.getCreepsInRange(1000, _tmpPoint, Targeting.getOldStyleTargets(targetMode));
			...
			if(_hasTarget)
			{
				if(!_targetCreep)
				{
					if(_behaviour === k_sBHVR_HUNT && (CREATURES._creatureCount > 0 || CREATURES._hasLivingGuardian) && _frameNumber % 150 == 0){
						...
						||
						(_targetBuilding.health <= 0)
						||
						(_targetGroup == 3 && _targetBuilding._looted)
						||
						(_targetBuilding._class == "tower" && !MONSTERBUNKER.isBunkerBuilding(_targetBuilding._type) && (_targetBuilding as BTOWER).isJard)){
						loseTarget();
						findTarget(_targetGroup);
					}
				}
