		...
		override public function Explode() : void
		{
			...
			for(_loc3_ in _loc7_)
			{
				...
				if(_loc2_.health > 0)
				{
					...
					_loc4_ = Number(_loc1_.dist);
					_loc5_ = _loc1_.pos;
					_loc2_.modifyHealth(-(damageProperty.value / _buildingProps.size * (_buildingProps.size - _loc4_ * 0.5)));
					if(_loc2_.health <= 0)
					{
						_loc9_++;
			...
			for(_loc3_ in _loc7_)
			{
				...
				if(_loc2_.health > 0)
				{
					...
					_loc4_ = Number(_loc1_.dist);
					_loc5_ = _loc1_.pos;
					_loc2_.modifyHealth(-(damageProperty.value * 0.5 / _buildingProps.size * (_buildingProps.size - _loc4_ * 0.5)));
					if(_loc2_.health <= 0)
					{
						_loc9_++;
						GIBLETS.Create(new Point(_mc.x,_mc.y + 3),0.8,75,2);
