	...
	try {
		...
		for (const dbCell of dbCells) {
			...
			if (dbCell.uid && isDefensiveStructure(dbCell.base_type)) {
				...
				if(dbCell.base_type === EnumYardType.PLAYER && dbCell.save){
					defenderLevels = getDefenderLevels(dbCell.base_type, getPlayerDefenderLevel(dbCell.save.level));
				} else {
					defenderLevels = getDefenderLevels(dbCell.base_type);
				}
