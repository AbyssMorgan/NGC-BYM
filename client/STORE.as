	...
	public class STORE {
		...
		public static function Variables(): void {
			...
			while (resourceIndex <= 4) {
				...
				if (BASE._resources["r" + resourceIndex].Get() + BASE._resources["r" + resourceIndex + "max"] * 0.1 < BASE._resources["r" + resourceIndex + "max"]) {
					...
					mainStoreItems.d = KEYS.Get("str_top_10pct_noroom", {
						"v1": KEYS.Get(GLOBAL._resourceNames[resourceIndex - 1])
					});
					mainStoreItems.c = [0];
				...
				if (Boolean(BASE._iresources) && BASE._iresources["r" + resourceIndex].Get() + BASE._iresources["r" + resourceIndex + "max"] * 0.1 < BASE._iresources["r" + resourceIndex + "max"]) {
					...
					infernoStoreItems.d = KEYS.Get("str_top_10pct_noroom", {
						"v1": KEYS.Get(GLOBAL.iresourceNames[resourceIndex - 1])
					});
				...
				if (BASE._resources["r" + resourceIndex].Get() + BASE._resources["r" + resourceIndex + "max"] * 0.5 < BASE._resources["r" + resourceIndex + "max"]) {
					...
					mainStoreItems.d = KEYS.Get("str_top_50pct_noroom", {
						"v1": KEYS.Get(GLOBAL._resourceNames[resourceIndex - 1])
					});
					mainStoreItems.c = [0];
					mainStoreItems.quantity = 0;
				...
				if (infernoStoreItems) {
					...
					if (BASE._iresources["r" + resourceIndex].Get() + BASE._iresources["r" + resourceIndex + "max"] * 0.5 < BASE._iresources["r" + resourceIndex + "max"]) {
						...
						infernoStoreItems.d = KEYS.Get("str_top_50pct_noroom", {
							"v1": KEYS.Get(GLOBAL.iresourceNames[resourceIndex - 1])
						});
						infernoStoreItems.c = [0];
						infernoStoreItems.quantity = 0;
				...
				mainStoreItems.t = KEYS.Get("str_top_fill_label", {
					"v1": KEYS.Get(GLOBAL._resourceNames[resourceIndex - 1])
				});
				if (BASE._resources["r" + resourceIndex + "max"] > BASE._resources["r" + resourceIndex].Get()) {
					...
					mainStoreItems.d = KEYS.Get("str_resourcesfull", {
						"v1": KEYS.Get(GLOBAL._resourceNames[resourceIndex - 1])
					});
					mainStoreItems.c = [0];
					mainStoreItems.quantity = 0;
				...
				if (infernoStoreItems) {
					infernoStoreItems.t = KEYS.Get("str_top_fill_label", {
						"v1": KEYS.Get(GLOBAL.iresourceNames[resourceIndex - 1])
					});
				}
				if (BASE._iresources["r" + resourceIndex + "max"] > BASE._iresources["r" + resourceIndex].Get()) {
					...
					infernoStoreItems.d = KEYS.Get("str_resourcesfull", {
						"v1": KEYS.Get(GLOBAL.iresourceNames[resourceIndex - 1])
					});
					infernoStoreItems.c = [0];
					infernoStoreItems.quantity = 0;
			...
			_storeItems.FIX.d = KEYS.Get("desc_repairbdgs", {
				"v1": _repairCount
			});
			...
			if (_loc8_) {
				...
				_storeItems.HAMS.d = KEYS.Get("str_healmons_desc", {
					"v1": _loc8_
				});
			...
			if (BASE.isInfernoMainYardOrOutpost) {
				...
				_storeItems.BLK2.resourceCost = _loc11_;
			...
			if (BASE.isInfernoMainYardOrOutpost) {
				...
				_storeItems.BLK3.resourceCost = _loc11_;
			...
			if (!BASE.isInfernoMainYardOrOutpost) {
				...
				_storeItems.BLK4.resourceCost = _loc11_;
			...
			if (!BASE.isInfernoMainYardOrOutpost) {
				...
				_storeItems.BLK5.resourceCost = _loc11_;
			...
			if (!BASE.isInfernoMainYardOrOutpost) {
				...
				_storeItems.BLK6.resourceCost = _loc11_;
			...
			if (!BASE.isInfernoMainYardOrOutpost) {
				...
				for each(_loc13_ in _loc18_) {
					if (_loc13_._lvl.Get() <= 1) {
						_loc11_ += resources_blk7 + resources_blk6 + resources_blk5 + resources_blk4 + resources_blk3 + resources_blk2;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 2) {
						_loc11_ += resources_blk7 + resources_blk6 + resources_blk5 + resources_blk4 + resources_blk3;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 3) {
						_loc11_ += resources_blk7 + resources_blk6 + resources_blk5 + resources_blk4;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 4) {
						_loc11_ += resources_blk7 + resources_blk6 + resources_blk5;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 5) {
						_loc11_ += resources_blk7 + resources_blk6;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 6) {
						_loc11_ += resources_blk7;
						_loc12_ += 1;
					}
				}

				_storeItems.BLK7.c = [0];
				_storeItems.BLK7.resourceCost = _loc11_;
				_storeItems.BLK7.d = KEYS.Get("desc_scourgewalls", {
					"v1": _loc12_,
					"v2": GLOBAL.FormatNumber(_loc11_)
				});
				_storeItems.BLK7.t = KEYS.Get("str_scourgewalls");
			}
			...
			if (!BASE.isInfernoMainYardOrOutpost) {
				...
				for each(_loc13_ in _loc18_) {
					if (_loc13_._lvl.Get() <= 1) {
						_loc11_ += resources_blk8 + resources_blk7 + resources_blk6 + resources_blk5 + resources_blk4 + resources_blk3 + resources_blk2;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 2) {
						_loc11_ += resources_blk8 + resources_blk7 + resources_blk6 + resources_blk5 + resources_blk4 + resources_blk3;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 3) {
						_loc11_ += resources_blk8 + resources_blk7 + resources_blk6 + resources_blk5 + resources_blk4;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 4) {
						_loc11_ += resources_blk8 + resources_blk7 + resources_blk6 + resources_blk5;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 5) {
						_loc11_ += resources_blk8 + resources_blk7 + resources_blk6;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 6) {
						_loc11_ += resources_blk8 + resources_blk7;
						_loc12_ += 1;
					}
					if (_loc13_._lvl.Get() == 7) {
						_loc11_ += resources_blk8;
						_loc12_ += 1;
					}
				}

				_storeItems.BLK8.c = [0];
				_storeItems.BLK8.resourceCost = _loc11_;
				_storeItems.BLK8.d = KEYS.Get("desc_reaverwalls", {
					"v1": _loc12_,
					"v2": GLOBAL.FormatNumber(_loc11_)
				});
				...
				if (GLOBAL.mode == GLOBAL.e_BASE_MODE.BUILD && Boolean(_loc2_)) {
					...
					if (_loc3_ > 0) {
						if (_streamline_cost == 0) {
							...
							_streamline.tDescription.htmlText = KEYS.Get("streamspd_desc", {
								"v1": GLOBAL.ToTime(_streamline_time, false, false)
							});
							...
							_streamline.mcInstant.bAction.Setup(KEYS.Get("btn_useshiny", {
								"v1": _streamline_cost
							}));
						}
						...
						if (CREATURELOCKER._unlocking != null) {
							...
							if (_loc3_ > 0) {
								if (param1 == "SP1") {
									...
									_streamline.tDescription.htmlText = KEYS.Get("str_closeenough_unlock", {
										"v1": _loc4_
									});
									if (_loc3_ <= 60 * 5) {
										_streamline.tDescription.htmlText = KEYS.Get("str_closeenough_unlock", {
									...
									_streamline.tDescription.htmlText = KEYS.Get("str_30minutes_unlockdesc", {
										"v1": _loc4_
									});
								} else if (param1.substr(0, 3) == "SP3") {
									_streamline.tTitle.htmlText = KEYS.Get("str_60minutes_unlocklabel");
									_streamline.tDescription.htmlText = KEYS.Get("str_60minutes_unlockdesc", {
										"v1": _loc4_
									});
								} else if (param1.substr(0, 3) == "SP4") {
									_streamline.tTitle.htmlText = KEYS.Get("str_finishnow_unlocklabel", {
										"v1": _loc4_
									});
						...
						if (ACADEMY._monsterID != null) {
							...
							if (_loc3_ > 0) {
								if (param1 == "SP1") {
									_streamline.tTitle.htmlText = KEYS.Get("str_closeenough");
									_streamline.tDescription.htmlText = KEYS.Get("str_closeenough_traindesc", {
										"v1": _loc4_
									});
									if (_loc3_ <= 60 * 5) {
										_streamline.tDescription.htmlText = KEYS.Get("str_closeenough_traindesc_ok", {
											"v1": _loc4_
										});
									}
								} else if (param1.substr(0, 3) == "SP2") {
									_streamline.tTitle.htmlText = KEYS.Get("str_30minutes_trainlabel");
									_streamline.tDescription.htmlText = KEYS.Get("str_30minutes_traindesc", {
										"v1": _loc4_
									});
								} else if (param1.substr(0, 3) == "SP3") {
									_streamline.tTitle.htmlText = KEYS.Get("str_60minutes_trainlabel");
									_streamline.tDescription.htmlText = KEYS.Get("str_60minutes_traindesc", {
										"v1": _loc4_
									});
								} else if (param1.substr(0, 3) == "SP4") {
									_streamline.tTitle.htmlText = KEYS.Get("str_finishnow_trainlabel", {
										"v1": _loc4_
									});
									_streamline.tDescription.htmlText = KEYS.Get("str_finishnow_unlocktimesave", {
										"v1": GLOBAL.ToTime(_loc3_, false, false),
						...
						if ((_loc5_ = _loc2_ as MONSTERLAB)._upgrading != null) {
							...
							if (_loc3_ > 0) {
								if (param1 == "SP1") {
									...
									_streamline.tTitle.htmlText = KEYS.Get("str_30minutes_poweruplabel", {
										"v1": _loc6_
									});
									...
									_streamline.tTitle.htmlText = KEYS.Get("str_60minutes_poweruplabel", {
										"v1": _loc6_
									});
									...
									_streamline.tTitle.htmlText = KEYS.Get("str_finishnow_poweruplabel", {
										"v1": _loc6_
									});
									_streamline.tDescription.htmlText = KEYS.Get("str_finishnow_unlocktimesave", {
										"v1": GLOBAL.ToTime(_loc3_, false, false),
		...
		public static function CalcCost(param1: String, param2: Boolean = false): * {
			...
			if (_loc9_.substr(0, 3) == "BLK") {
				...
				} else if (!BASE.isOutpost && _loc26_ < 18 && _loc9_.substr(3, 1) == "7") {
					_loc20_ = KEYS.Get("upgradeth", {
						"v1": 18
					});
		...
		public static function Switch(param1: int, param2: Number, param3: Array = null, param4: Boolean = false): void {
			...
			var block_level_7: Boolean = false;
			var block_level_8: Boolean = false;
			...
			while (index < groupArray.length) {
					...
					if (_loc28_) {
						...
						if (_loc24_ > 0) {
							...
							if (item == "SP1") {
								...
								storeItemObject.t = KEYS.Get("str_finishnow_desc", {
									"v1": _loc26_
								});
							...
							if (CREATURELOCKER._unlocking != null) {
								...
								if (_loc24_ > 0) {
									if (item == "SP1") {
										storeItemObject.t = KEYS.Get("str_closeenough");
										storeItemObject.d = KEYS.Get("str_closeenough_unlock", {
											"v1": _loc25_
										});
										if (_loc24_ <= 60 * 5) {
											storeItemObject.d = KEYS.Get("str_closeenough_unlock", {
										...
										storeItemObject.d = KEYS.Get("str_30minutes_unlockdesc", {
											"v1": _loc25_
										});
									} else if (item.substr(0, 3) == "SP3") {
										storeItemObject.t = KEYS.Get("str_60minutes_unlocklabel");
										storeItemObject.d = KEYS.Get("str_60minutes_unlockdesc", {
											"v1": _loc25_
										});
									} else if (item.substr(0, 3) == "SP4") {
										storeItemObject.t = KEYS.Get("str_finishnow_unlocklabel", {
											"v1": _loc25_
										});
							...
							if (ACADEMY._monsterID != null) {
								...
								if (_loc24_ > 0) {
									if (item == "SP1") {
										storeItemObject.t = KEYS.Get("str_closeenough");
										storeItemObject.d = KEYS.Get("str_closeenough_traindesc", {
											"v1": _loc25_
										});
										if (_loc24_ <= 60 * 5) {
											storeItemObject.d = KEYS.Get("str_closeenough_traindesc_ok", {
												"v1": _loc25_
											});
										}
									} else if (item.substr(0, 3) == "SP2") {
										storeItemObject.t = KEYS.Get("str_30minutes_trainlabel");
										storeItemObject.d = KEYS.Get("str_30minutes_traindesc", {
											"v1": _loc25_
										});
									} else if (item.substr(0, 3) == "SP3") {
										storeItemObject.t = KEYS.Get("str_60minutes_trainlabel");
										storeItemObject.d = KEYS.Get("str_60minutes_traindesc", {
											"v1": _loc25_
										});
									} else if (item.substr(0, 3) == "SP4") {
										storeItemObject.t = KEYS.Get("str_finishnow_trainlabel", {
											"v1": _loc25_
										});
										storeItemObject.d = KEYS.Get("str_finishnow_unlocktimesave", {
											"v1": GLOBAL.ToTime(_loc24_, false, false),
							...
							if ((_loc34_ = _loc28_ as MONSTERLAB)._upgrading != null) {
								...
								if (_loc24_ > 0) {
									if (item == "SP1") {
										...
										storeItemObject.t = KEYS.Get("str_30minutes_poweruplabel", {
											"v1": _loc35_
										});
										...
										storeItemObject.t = KEYS.Get("str_60minutes_poweruplabel", {
											"v1": _loc35_
										});
										...
										storeItemObject.t = KEYS.Get("str_finishnow_poweruplabel", {
											"v1": _loc35_
										});
										storeItemObject.d = KEYS.Get("str_finishnow_unlocktimesave", {
											"v1": GLOBAL.ToTime(_loc24_, false, false),
				...
				if (item.substr(0, 3) == "BLK") {
					...
					if (!BASE.isOutpost && town_hall_level < 3 && item.substr(3, 1) == "2") {
					...
					} else if (!BASE.isOutpost && town_hall_level < 18 && item.substr(3, 1) == "7") {
						_loc32_ = KEYS.Get("upgradeth", {
							"v1": 18
						});
					} else if (!BASE.isOutpost && town_hall_level < 20 && item.substr(3, 1) == "8") {
						_loc32_ = KEYS.Get("upgradeth", {
							"v1": 20
						});
						...
						for each(_loc42_ in _loc41_) {
							...
							if (!BASE.isInfernoMainYardOrOutpost) {
								...
								if (_loc42_._lvl.Get() == 6 && town_hall_level >= 18) {
									block_level_7 = true;
									block_level_8 = true;
								}
								if (_loc42_._lvl.Get() == 7 && town_hall_level >= 20) {
									block_level_8 = true;
								}
							}
						}
				...
				if (_loc32_ != "") {
					...
					if (_loc6_ >= _loc7_ && !storeItemObject.i || item.substr(0, 3) == "BEW" && QUEUE._workerCount >= UI_WORKERS._maxWorkers || item.substr(0, 2) == "BR" && BASE._resources["r" + item.substr(2, 1)].Get() >= BASE._resources["r" + item.substr(2, 1) + "max"] && item.substr(item.length - 1) != "I" || item.substr(0, 2) == "BR" && !BASE.isInfernoMainYardOrOutpost && item.substr(item.length - 1) == "I" && BASE._iresources["r" + item.substr(2, 1)].Get() >= BASE._iresources["r" + item.substr(2, 1) + "max"]) {
						...
						if (Boolean(_storeInventory[_loc31_]) && _storeInventory[_loc31_].Get() > 0) {
							_loc9_ = "<font color=\"#0000ff\"><b>" + _storeInventory[_loc31_].Get() + "</b></font>";
							_loc10_ = "<font color=\"#0000CC\">" + KEYS.Get("store_inyourinventory", {
								"v1": _storeInventory[_loc31_].Get()
							}) + "</font>";
						} else {
							_loc9_ = "<font color=\"#CC0000\">" + KEYS.Get("str_prob_soldout") + "</font>";
							if (storeItemObject.du > 0) {
								_loc10_ = "<font color=\"#CC0000\">" + KEYS.Get("store_rebuy", {
									"v1": GLOBAL.ToTime(_storeData[item].e - GLOBAL.Timestamp(), true)
								}) + "</font>";
							} else if (_loc6_ == 0) {
						...
						if (Boolean(_storeInventory[_loc31_]) && Boolean(_storeInventory[_loc31_].Get())) {
							...
							_loc9_ = _loc33_;
							if (storeItemObject.du > 0) {
								_loc10_ = "<font color=\"#0000CC\">" + KEYS.Get("str_buy_cooldown", {
									"v1": GLOBAL.ToTime(storeItemObject.du)
								}) + "</font>";
								...
								if (_loc6_ == 0 && _loc7_ > 1) {
									_loc10_ = "<font color=\"#0000CC\">" + KEYS.Get("str_buy_xtimes", {
										"v1": _loc7_
									}) + "</font>";
								}
								if (_loc6_ > 0) {
		...
		public static function BuyC(param1: String, param2: int): void {
			...
			if (TUTORIAL._stage > 200) {
				...
				switch (itemCode) {
					case "BEW":
						arr = [KEYS.Get("str_code_bew_title"), KEYS.Get("str_code_bew_body", {
				...
				if (arr.length > 0) {
					...
					GLOBAL.Message(KEYS.Get("msg_purchase_complete", {
						"v1": _storeItems[itemCode].t
					}));
				}
			}
		}
