package com.monsters.siege.weapons
{
	...
	public class VacuumHose implements IAttackable, ITickable
	{
		...
		public function VacuumLoot(param1:int) : void
		{
			...
			var _loc6_:int = 0;
			var force_inferno_resources:Boolean = (_target._type == 153);
			if(!this.hasMaxedAmountToLoot())
			{
				...
				if(force_inferno_resources){
					...
					{
						_loc4_.push({"id":1, "quantity":BASE._iresources.r1.Get()});
					}
					if(BASE._iresources.r2.Get() > 0)
					{
						_loc4_.push({"id":2, "quantity":BASE._iresources.r2.Get()});
					}
					if(BASE._iresources.r3.Get() > 0)
					{
						_loc4_.push({"id":3, "quantity":BASE._iresources.r3.Get()});
					}
					if(BASE._iresources.r4.Get() > 0)
					{
						_loc4_.push({"id":4, "quantity":BASE._iresources.r4.Get()});
					}
				} else {
				...
				if(_loc4_.length > 0)
				{
					...
					ATTACK.Loot(_loc5_.id, _loc3_, this._target.x, this._target.y, 9, this._target, true, force_inferno_resources);
		...
		public function ApplyVacuum(param1:int, param2:int) : void
		{
			...
			if(force_inferno_resources){
				this._totalPossibleLoot = BASE._iresources.r1.Get() + BASE._iresources.r2.Get() + BASE._iresources.r3.Get() + BASE._iresources.r4.Get();
			} else {
				this._totalPossibleLoot = BASE._resources.r1.Get() + BASE._resources.r2.Get() + BASE._resources.r3.Get() + BASE._resources.r4.Get();
			}
