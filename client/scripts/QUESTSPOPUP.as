	...
	public class QUESTSPOPUP extends QUESTSPOPUP_CLIP
	{
		...
		public function ListQuests(param1:MouseEvent = null) : void
		{
			...
			if(param1)
			{
				...
				offset = 0;
				while(offset < QUESTS._quests.length)
				{
					quest = QUESTS._quests[offset];
					if(quest.group == this._groupID && QUESTS._completed && QUESTS._completed[quest.id] == 1)
					{
						this.ShowQuestB(quest.id);
						break;
					}
					offset++;
				}
		...
		public function ListQuestsB(param1:int) : void
		{
			...
			var sortedQuests:Array = QUESTS._quests.slice();
			sortedQuests.sortOn("order", Array.NUMERIC);
			c = 0;
			if(QUESTS._completed)
			{
				i = 0;
				while(i < sortedQuests.length)
				{
					q = sortedQuests[i];
