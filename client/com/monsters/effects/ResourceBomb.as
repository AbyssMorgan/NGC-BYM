package com.monsters.effects
{
	...
	public class ResourceBomb
	{
		...
		public function ResourceBomb(param1:MovieClip, param2:Point, param3:Object, param4:int)
		{
			...
			{
				for each(_loc16_ in CREEPS._creeps)
				{
		...
		public function Damage(param1:Point) : void
		{
			...
			if(this.resourceid !== ResourceBombParticle.k_TYPE_PUTTY)
			{
				for each(building in this.targets)
				{
					_loc6_ = building[0];
					_loc7_ = (building[1] * 0.5) + 0.5;
					_loc4_ = int(_loc7_ * this.dpp);
					if(_loc6_._type == 114){
						_loc4_ = 0;
					} else if(_loc6_._class == "wall"){
						_loc4_ *= 0.06;
					} else if(_loc6_._class == "tower"){
						_loc4_ *= 0.9;
